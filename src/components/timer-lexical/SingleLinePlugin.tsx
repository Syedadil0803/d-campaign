 ...
 * the last-good EditorState snapshot and reverts to it when an edit GREWS the
 * text onto a 2nd line (deletions are always allowed, so a pre-existing 2-line
 * timer can be edited back down). Skips tick/sync updates via tags. Calls
 * `onOverflow` so the host can show its "field limit reached" warning.
 */
...
import { wrapsAtWidth, TIMER_MAX_CONTENT_WIDTH } from './lineMeasure';
...
export function SingleLinePlugin({
  ...
  useEffect(() => {
    ...
    return editor.registerUpdateListener(({ editorState, tags }) => {
      ...
      // Tick / programmatic syncs are not user edits → never cap them.
      if (
        tags.has('timer-tick') ||
      ...
      requestAnimationFrame(() => {
        ...
        const grew = lastGoodLen.current >= 0 && len > lastGoodLen.current;
        ...
        if (grew && wrapsAtWidth(root, maxContentWidth) && lastGoodState.current) {
          // Reject: this edit grew the text onto a 2nd line → revert.
